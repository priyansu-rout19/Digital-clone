...
core/           ← all runtime code
  ...
  langgraph/    ← 18-node orchestration graph (Component 04)
...
| **TEI** for embedding (production) | **OpenAI API** (dev fallback, 1024-dim truncated) | Same hardware dependency; same schema, zero migration needed when TEI arrives |
...
| **Mem0** for cross-session user memory | Not implemented (stub) | Depends on vector backend being confirmed (pgvector confirmed, Zvec pending); next task |
