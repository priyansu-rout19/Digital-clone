...
**Date:** March 5, 2026 (Session 23, updated Session 25) | **Audited by:** 3 parallel agents scanning 16+ files, 2000+ lines
...
|--------|-------------|------------|---------|---------|------------|
| ParaGPT | 9 | 8 | 1 | 0 | 96% |
| Sacred Archive | 9 | 6 | 2 | 1 | 83% |
| **Combined** | **18** | **14** | **3** | **1** | **89%** |

**12 gaps found.** 10 are fixable now (no PCCI dependency). 2 are PCCI-blocked.
...
| 6 | Confidence-Aware Responses (hedge honestly) | ✅ DONE | `routing_nodes.py` soft_hedge_router, threshold=0.80 | — |
...
| 4 | Out-of-corpus → "here's what I've said about [related]" | ⚠️ | Hedges but **doesn't suggest related topics** |
...
| 2 | Silence Mode (reverent + suggest related) | ⚠️ PARTIAL | `make_strict_silence_router()`, threshold=0.95 | **No dynamic topic suggestions.** Message text **FIXED Session 24** — uses institutional voice per SOW. |
...
| 4 | Mandatory Human Review (approve/reject/edit) | ⚠️ PARTIAL | `review_queue_writer`, `review.py` approve/reject | **EDIT action not implemented** |
| 5 | Access Tier System (devotee, friend, follower) | ✅ DONE | AccessTier enum, SQL filtering, UI tier selector | — |
| 6 | Review Dashboard (sources, confidence, keyboard shortcuts) | ⚠️ PARTIAL | `review/Dashboard.tsx` shows question+response+confidence | **Missing:** cited sources display, keyboard shortcuts, EDIT |
...
| 3 | Untaught topic → Silence Mode + related topics | ⚠️ | Silence triggers, **no topic suggestions** |
| 4 | Cross-topic → knowledge graph traversal | ✅ | Provenance CTE traversal works |
| 5 | Reviewer rejects → logged, seeker gets silence | ❌ | **No rejection→seeker notification flow** |
| 6 | Batch 60 responses/day with keyboard shortcuts | ❌ | **No keyboard shortcuts, mouse-only** |
...
| Review throughput | 50+/day | ❌ Not achievable | No keyboard shortcuts |
...
### P1 — SOW Requirements (explicitly stated)

| # | Gap | Clients | Fix | Effort |
|---|-----|---------|-----|--------|
| 4 | **Review EDIT action** — only approve/reject, no edit | Sacred Archive | Add PUT endpoint in `review.py`, add textarea + save in Dashboard | Small |
| 5 | **Review keyboard shortcuts** — mouse-only, no batch efficiency | Sacred Archive | Add `onKeyDown` handlers in `Dashboard.tsx` (a=approve, r=reject, e=edit) | Small |
| 6 | **Review show cited sources** — dashboard doesn't display source passages | Sacred Archive | Render CitationCards in Dashboard.tsx alongside each review item | Small |
| 7 | **Dynamic topic suggestions in silence** — static message, no related topics | Both | Extract topic names from retrieved passages, interpolate into silence message | Medium |
...
### Phase 2: P1 Fixes (Review Dashboard Enhancement)

| File | Change |
|------|--------|
| `api/routes/review.py` | Add PUT endpoint for editing response text before approval |
| `ui/src/pages/review/Dashboard.tsx` | Add: cited sources panel, edit textarea, keyboard shortcuts (a/r/e/Tab), batch counter |
| `ui/src/api/types.ts` | Update ReviewItem interface to include `cited_sources` |
| `ui/src/api/client.ts` | Add `editReviewItem()` function |

### Phase 3: P2 Fixes (Audit + Security)

| File | Change |
|------|--------|
| `api/routes/review.py` | INSERT to AuditLog on approve/reject/edit actions |
| `api/routes/ingest.py` | INSERT to AuditLog on document upload |
| `api/routes/users.py` | Add auth middleware, INSERT to AuditLog on data deletion |
| `core/langgraph/nodes/routing_nodes.py` | Dynamic topic suggestions: extract topic names from passages, interpolate into silence message |
