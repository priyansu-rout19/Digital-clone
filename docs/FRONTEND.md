...
| File | Purpose |
|------|---------|
| `paragpt/Landing.tsx` | Profile card (avatar, name, bio, topic tags), 3 starter questions (corpus-aligned: ASEAN, infrastructure, + irrelevant hedge demo), sticky input bar with model selector |
...
### WebSocket Protocol
```
Client → Server: { query, user_id, access_tier, model }
Server → Client: { type: "progress", node: "query_analyzer" }  // repeated per node
Server → Client: { type: "response", response, confidence, cited_sources, model, ... }
